...
- **각도 한계 `GeckoMotor.HEAD_LIMIT` = ±8°** (떠돌기 + 둘러보기 + 바라보기 합) — 최종 그림으로 머리만 돌려 보니 위로 10° 넘게 들면 **턱 밑 목선(머리 그림이 곧게 잘린 곳)과 작은 틈**이, 아래로 10° 넘게 숙이면 **등 돌기가 두 겹**으로 보였다. 동작(올려다보기 16° · 하품 14° 등)은 이 제한 밖이라 짧게 그 이음새가 보일 수 있다 — 그림에서 고칠 일 (`ART_ORDER_GECKO.md` 목·턱 여유)
