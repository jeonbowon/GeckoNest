...
| 장식 | 칸 | 게코 |
|------|------|------|
| 동굴 · 하이드 하우스 | 바닥 | 동굴은 **문으로 들어간다**, 집은 문이 작아 뒤에 숨는다 (2026-09-21, 아래 "은신처 문") → 5~12초(졸리면 12~25초) 엎드려 쉼 → 돌아서서 나옴. 누르면 "누구야?" 하고 나옴, 돌봄 버튼·꼬리 누르기도 나오게 함 |
...
**은신처 문 (2026-09-21, `GeckoMovementAI.GoHide · PlanDoor`, `DecorItemSO.doorRect`):** 예전에는 은신처 그림을 게코 앞에 그려 가렸는데, 다 자란 게코(길이 640)가 은신처(460)보다 길어 **머리가 반대편으로 삐져나와** 굴에 들어가는 게 아니라 바위 뒤에 숨은 것처럼 보였다. 이제 동굴은 **문으로 들어간다**
- 문 = 그림 안 비율 사각형 (`doorRect`, 왼쪽 아래 0,0). 홈 화면이 놓인 자리·원근·반전을 반영해 영역 좌표로 넘긴다 (`HomeUIController.DoorRectOf` → `Structure.door`)
- 순서: 문 앞에 서서(주둥이가 문 가장자리 8 앞) 문을 본다 → 은신처는 게코 뒤로, **문 가장자리 너머는 게코 오브젝트의 `RectMask2D`로 잘라 낸다**(가장자리 18 흐리게 — 어둠 속으로 스며들듯) → 고개를 6° 숙이고 꼬리를 늘어뜨리며(`GeckoMotor.SetBurrowed`) 0.5배 걸음으로 몸의 68%가 문 안에 들 때까지 → 엎드림 → 나올 때는 안에서 돌아서서(머리가 밖으로) 문 앞까지
- **크기는 바꾸지 않는다** — 문이 게코보다 작아 들어가면서 작아지고 납작해지게 했더니 "갑자기 작아져서 이상하다"(2026-09-21). 크기를 그대로 두면 문보다 큰 몸이 문 위 바위 속으로 파고드는 모양이 된다 → **문이 게코만큼 커야 한다**: 동굴 임시 그림(`DecorProxyArt.DrawCave`)의 입구를 곧은 옆면 230 + 둥근 위 45(폭 280)로 다시 그렸다 (`doorRect` 0.196~0.804 × 0.10~0.698, 높이 275 ≥ 게코 키 234)
- **집(`decor_hide`)은 문이 게코 키의 35%라 문 정보를 비워 둔다** → 뒤에 숨기(`GoHideBehind`): 집 그림이 게코 앞, 주둥이가 집 폭의 75% 안쪽에 머무는 곳에서 멈춰 반대편으로 삐져나오지 않고, 엎드려 꼬리만 삐죽. 문이 큰 새 그림이 오면 `doorRect`만 채우면 된다 (`ART_ORDER_GECKO.md` 8장)
- 들어갈 쪽: 문이 은신처 한쪽에 치우쳐 있으면 그쪽 바깥에서, 가운데면 화면 가운데 쪽에서, 그쪽에 설 자리가 없으면 반대쪽에서
- 문 안에 있을 때 게코를 누르면: 꼬리는 평소대로(끝 = 달아남), 그 밖은 집을 누른 것처럼 "누구야?" 하고 나온다
- 수치 Inspector `GeckoMovementAI` 은신처 항목(`doorInside` 0.68 · `doorSoftness` 18) [TBD]
