...
## 2026-09-21 — Game 창 잘림 · 배경·바닥을 "테마" 하나로

### Game 창이 잘려 보임 (`HakoGameViewFit`)

홈에서 게코 이름·코인·젬·돌봄 버튼·하단 탭이 사라지고 좌우가 잘려 보였다. 씬(`MainHome.unity`, 마지막 변경 2026-09-18)과
코드는 그대로였고, 원인은 **Game 창 상태**였다 — 창 상태 파일(`%APPDATA%\Unity\Editor-5.x\Preferences\Layouts\current\default-6000.dwlt`)에
`m_Scale 1.2208` · `m_Translation (396.9, 795.4)` · `Free Aspect`. 1.22배로 확대한 채 옆으로 밀려 있어 가장자리가 화면 밖으로 나갔다
(Game 창 위에서 Ctrl/Alt + 휠로 확대되고, Unity를 다시 켜도 남는다).
→ `Editor/HakoGameViewFit`: 1080×2400 고정 해상도를 목록에 넣어 고르고 확대를 화면에 딱 맞게 되돌린다.
처음 컴파일될 때 한 번 저절로, 이후 메뉴 `Hako > 화면 > 게임 화면 맞추기 (1080x2400)`. Game 창은 Unity 내부라 리플렉션.

### 배경·바닥 → 테마

| 문제 | 확인한 것 |
|------|------|
| 바닥이 거의 안 보인다 | `TerrariumFloor`는 화면 맨 아래 높이 150 — 하단 탭(120)과 돌봄 버튼이 덮는다. 80코인짜리 나무판 바닥을 사도 차이가 안 보였다 |
| 게코가 바닥 위를 걷지 않는다 | 발 높이 380~950 = **배경 그림 위** |
| 섞으면 어긋난다 | 배경은 이미 한 장짜리 장면(사막은 아래에 땅까지 그려져 있음) — 사막 배경 + 정글 흙처럼 조합 4개 중 2개가 어색 |
| 정글이 벽지처럼 보인다 | 정글 배경은 위에서 아래까지 잎사귀 벽이라 바닥 면이 없어, 게코·바위·동굴이 떠 보였다 |

→ **테마 = 뒷벽과 바닥이 한 장에 그려진 그림.** 아이디는 예전 배경 그대로(`bg_*`)라 저장을 옮기지 않는다.

- 꾸미기 화면: 바닥 탭 숨김(탭 줄이 가로 배치라 테마·장식 두 탭이 넓어짐), 배경 탭 글자 "테마"(`TerrariumUIController.ApplyThemeTabs`, 씬은 그대로). 번역 `terrarium.theme` · 테마 이름 "정글 테마"/"사막 테마"
- 홈: 바닥 띠(`_floorImage`)를 끈다
- `TerrariumManager`: `SetFloor` 삭제, `IsOwned`는 테마만 (바닥은 샀던 기록이 있어도 보유 아님)
- 저장 버전 10: 산 유료 바닥(`TerrariumData.RETIRED_FLOORS` — 나무판 80)은 코인으로 돌려주고 기록을 지운다. `floorId`는 예전 저장을 읽으려고 필드만 남김
- 그림: `Editor/ThemeProxyArt` — 정글 배경 아래 44%(0~1060)에 **코드로 그린 흙 바닥**을 원근으로 깔아 `theme_jungle.png`, `bg_jungle` 테마가 쓴다.
  처음에는 흙 그림(`floor_soil`, 가로 띠)을 위아래로 되풀이했더니 조약돌 줄이 300px마다 겹쳐 **지층처럼** 보였고,
  이끼를 많이 넣으니 **위장무늬**, 색을 몇 단으로 끊으니 채널별로 끊겨 **분홍·보라 얼룩**이 생겨 셋 다 걷어냈다.
  지금은 반복 없는 잡음 흙 결 · 벽 밑동 쪽 은은한 이끼 · 멀수록 작은 자갈 · 벽에서 늘어진 풀 · 먼 땅 안개 · 앞쪽 어둡게
- 사막 테마는 원래 아래에 땅이 있어 그대로
- 최종 그림 규격: `ART_ORDER_GECKO.md` 8-1 (1080×2400, 땅 0~1060, 게코 발 380~950이 땅 위, 맨 아래 330은 버튼에 가림, 가운데 비움)

자가 검사 7개 추가 (바닥은 보유 아님 · 산 바닥 80 돌려주기 · 깔려 있던 바닥도 돌려주기 · 무료 바닥은 0 · 정글 테마 그림 연결 ·
저장 v10 한 번만 돌려주기 · 다시 읽어도 또 안 줌) — **모두 통과 (296개)**

### 배경이 반투명이었다 · 앞 잎사귀 빼기

- **흙 바닥이 보랏빛 회색으로 보였다** — 씬의 `Background` 이미지가 알파 0.588(처음 UI를 만들 때부터)이라 뒤의 카메라 기본 하늘색(0.19, 0.30, 0.47)이 41% 비쳤다.
  정글 잎도 처음부터 원래 그림(밝은 연두)보다 어둡고 푸르게 보이고 있었는데, 흙이 들어와서 드러났다
  → `HomeUIController.RefreshTerrarium`이 테마를 입힐 때 색을 흰색(불투명)으로 고정 (씬은 그대로). 어둡게 하고 싶으면 알파가 아니라 색으로
- **아래 양쪽 모서리의 검은 얼룩** = `TerrariumAtmosphere`의 앞 잎사귀 2장(짙은 초록 88%, 322×520). 대부분 돌봄 버튼·하단 탭에 가려 끝만 삐져나왔고,
  어두운 배경에서는 묻혔지만 밝은 흙 위에서 얼룩으로 튀었다 → 뺐다 (`FxSprites.Leaf`·`LeafShape`도 삭제). 비네트·먼지는 그대로

---

## 2026-09-21 — 장식 효과 · 장식 찾아가기 · 꾸미기 탭 폭

"장식을 사야 할 이유가 예뻐서 하나뿐"이었다 — 게코는 은신처·구조물만 쓰고 바위·화분·이끼 바위는 보기만 했고,
어떤 장식을 놓아도 게임 수치는 그대로였다. 사용자가 누를 수 있는 것도 숨은 게코 불러내기뿐이었다.

### 1. 장식 효과 (`Domain/DecorPerks`, `DecorItemSO.perk`)

실제 게코 습성에서 가져온 작은 효과. **놓여 있기만 하면** 생겨서 앱을 꺼 둔 동안에도 그대로 계산된다. 같은 효과는 겹치지 않는다. [TBD]

| 효과 | 장식 | 규칙 | 습성 |
|------|------|------|------|
| MoltRub | 이끼 바위 | 허물 성공률 +10% | 거친 곳에 비벼 허물을 벗는다 |
| Droplets | 화분 | 물 줄 때 목마름 +10 더 | 크레스티드는 잎의 물방울을 핥아 마신다 |
| Basking | 바위 | 건강 회복 0.5 → 0.75/h | 따뜻한 돌에서 몸을 데운다 |
| Shelter | 집 · 동굴 | 기분 감소 ×0.8 | 숨을 곳이 있으면 안심한다 |
| Play | 코르크 · 덩굴 · 가지 · 유목 | 쓰다듬기 애정도 +1 | 놀 거리 |

`GeckoManager`: 허물 성공률을 `MoltSuccessRate`로 뽑아(자가 검사가 같이 씀) 이끼 바위를 더하고, `GiveWater` · `Pet` · 시간 보정의 기분 감소 · 건강 회복에 한 줄씩.

### 2. 장식 찾아가기 (`GeckoMovementAI.VisitDecor · Visit`)

- **짧게 누르면 그 장식으로 간다** — 집은 들어가고(이미 안이면 "누구야?"), 벽 구조물은 타고, 바닥 장식은 옆에 선다. 벽 구조물도 누를 수 있게 했다 (예전에는 바닥 장식만 버튼이 있었다)
- 바닥 장식 옆 자리: 장식의 **화면 가운데 쪽**(`VisitX`, 막히면 바깥쪽), 장식보다 30 앞이라 겹침 순서에서 게코가 앞에 그려진다. 도착하면 장식을 보고 `DecorVisited` → 홈 화면이
  - 이끼 바위: 허물 80 이상이면 근질근질 두 번 + 허물 조각(`GeckoFx.RubFlakes`), 아니면 킁킁
  - 화분: 잎 물방울(`LeafDroplets`) + 할짝 두 번 — 물을 준 뒤 마시기가 끝나면 스스로 간다 (`WetPlant` → `VisitDroplets`)
  - 바위: 엎드려 6~10초 + 금빛 알갱이(`WarmGlow`)
- 스스로도 간다: 허물 준비 중이면 쉬고 나서 50%로 이끼 바위, 고른 장식이 바위면 몸 데우기. 쓸 일이 없는 바닥 장식(허물 전 이끼 바위, 물 안 준 화분)은 그냥 걷는다

### 3. 꾸미기 카드 · 탭

- 카드 가격 밑에 연두색 작은 줄로 효과 (`DecorSlotUI.PerkLabel`, 수치는 `DecorPerks`에서 그대로) — 가격 칸 높이 68에 두 줄이 들어간다
- **탭 폭 (전날 작업의 문제)**: 바닥 탭을 숨기면 두 탭이 넓어질 줄 알았는데, 탭 줄(HorizontalLayoutGroup)이 칸만 나누고 버튼 폭은 그대로 두는 설정이라 두 탭이 칸 왼쪽에 작게 붙었다 → `ApplyThemeTabs`가 `childControlWidth`를 켠다

자가 검사 13개 추가 (에셋 9종 효과 · 칸 종류 · 허물 +10% · 물 +10 · 기분 ×0.8 · 건강 +50% · 애정 +1 · 카드 문구 · 설 자리 3가지) — **모두 통과 (309개)**

---
## 2026-09-21 — 머리 움직임 (둘러보기 · 바라보기)

"머리가 고정돼 보인다" — 코드에는 머리 떠돌기·둘러보기가 있었지만 **±2.2° · ±2.5°**라 눈으로는 안 움직이는 것처럼 보였다.

### 얼마나 돌려도 되나 — 최종 그림으로 직접 그려 봄

파츠 PNG와 스킨의 관절 값(머리 관절 = 목 쪽 가장자리, 그림 안 7%·40%)으로 몸통 위에 머리만 돌려 합성해 확대했다.
- 위아래 **±8°까지는 자연스럽다**
- **위로 10° 넘게** 들면 턱 밑 목선(머리 그림이 곧게 잘린 곳)이 직선으로 드러나고 작은 틈이 생긴다
- **아래로 10° 넘게** 숙이면 뒷머리 등 돌기가 몸통 돌기와 두 겹으로 겹친다

→ 평소 움직임(떠돌기 + 둘러보기 + 바라보기)의 합을 `GeckoMotor.HEAD_LIMIT` ±8°로 자른다.
동작(올려다보기 16° · 하품 14°)은 이 제한 밖이라 짧게 보일 수 있다 — 그림에서 고칠 일로 `ART_ORDER_GECKO.md` 4장에 "목·턱 밑을 60px 이상 이어 그리기"를 적었다.

### 끊어서 움직이기 (`UpdateLook · StartLook`)

도마뱀은 머리를 계속 흔들지 않는다 — **탁 돌리고, 멈춰서 보고, 다른 곳을 본다.**
- 2.5~5.5초마다 위(40%, 눈도 위) · 아래 바닥(30%) · 앞 · 뒤 흘끗, 3.5~7.5°
- 돌리기 0.08초(SmoothDamp) → 멈춰서 보기 0.7~1.9초 → 40%는 돌아오지 않고 다른 곳을 한 번 더 → 돌아오기 0.22초
- 볼 때 목을 3px 빼고 위를 보면 2px 든다 (회전만 하면 인형 머리 같다), 떠돌기 ±3°
- 걷기·동작·엎드려 쉬기·졸기 중에는 앞을 본다 (동작이 시작되면 둘러보기를 거둔다)

### 바라보기 (`LookAt`)

- 빈 바닥을 **한 번** 누르면 그쪽을 본다 (`FloorTapCatcher.Tapped` — 두 번 톡톡의 첫 번째도) · 선물 상자가 나타나면 그쪽을 본다
- 방향은 `HeadAngleToward`(목 관절 기준) — 앞쪽이면 고개를 그 방향으로(±8°), 뒤쪽이면 고개는 조금만 들고 눈으로 흘끗
- 화면 좌표 → 스킨 좌표는 `GeckoRig.WorldToSkin` (`_visual` 역변환이라 좌우 반전·벽 회전·크기까지 되돌린다)

자가 검사 6개 추가 (한계 ±8° · 앞 위 → +8° · 앞 아래 → 살짝 숙임 · 뒤쪽 · 좌표 되돌리기 4자세 · 왼쪽을 볼 때 앞뒤) — **모두 통과 (315개)**

---
## 2026-09-21 — 목 휨 · 은신처 문으로 들어가기

### 1. 고개를 들면 목이 잘린 것처럼 보임 → 목을 휘게 (`UI/Gecko/GeckoNeckBend`)

머리는 목 관절을 중심으로 **통째로** 돌았다. 머리 그림 아래가 곧게 잘려 있어서 고개를 들면(쓰다듬기 하트 · 청소 올려다보기 16° · 하품 14°)
그 선이 몸통에서 떨어져 턱 밑에 틈이 생겼고, 숙이면 뒷머리 등 돌기가 몸통 돌기와 두 겹으로 보였다.
최종 그림으로 머리만 돌려 합성해 비교했다 — 통째로 14°면 턱 밑에 초록 틈, 목을 휘면 틈 없이 이어진다 (-14°도 돌기가 한 줄).

- 머리 Image에 메시 효과(`BaseMeshEffect`) — 그림을 세로 띠 18칸으로 나눠 **목 쪽(왼쪽 12%)은 몸통 각도로 되돌리고, 46%부터는 머리 각도 그대로**, 그 사이는 부드럽게
- 머리 오브젝트는 그대로 머리 각도만큼 돌아서 눈(53%)·입·혀 표정 그림이 어긋나지 않는다 (46%를 눈보다 앞으로 잡은 이유)
- `GeckoRig`가 머리에 자동으로 붙이고 매 프레임 머리 각도를 넣는다 (바뀔 때만 메시를 다시 만든다)
- 미리보기 스크립트에서 `[Math]::Min(1, x)`가 정수로 계산돼 휨이 0/1로 끊겨 찢어져 보였던 일이 있었다 (PowerShell 오버로드 — 게임 코드와 무관)

### 2. 은신처에 들어가는 게 아니라 뒤에 숨는 것처럼 보임 → 문으로 들어가기 (`GeckoMovementAI.GoHide · PlanDoor`)

예전에는 은신처 그림을 게코 앞에 그려 가렸는데, 다 자란 게코(길이 640)가 은신처(460)보다 길어 **머리가 반대편으로 삐져나왔다.**

| | 문 너비 | 문 높이 | 게코 키(234) 대비 |
|---|---|---|---|
| 동굴 | 210 | 150 | 64% |
| 집 | 97 | 83 | 35% |

- 문 = 그림 안 사각형 `DecorItemSO.doorRect` (동굴은 타원 문의 45% 높이 폭). 홈 화면이 놓인 자리·원근·반전을 반영해 영역 좌표로 (`DoorRectOf`)
- 문 앞에 서서 문을 보고 → 은신처는 게코 **뒤로**, 문 가장자리 너머는 게코 오브젝트의 **`RectMask2D`로 잘라 낸다** (게코 오브젝트는 바닥에서 회전·크기가 없어 영역 좌표 − 발 위치 = 로컬 x). 가장자리 18을 흐리게 해 칼로 자른 선 대신 어둠 속으로 스며들듯
- 문이 게코보다 작아 **들어가면서 움츠린다** — 몸 높이를 문 높이 × 0.9에: 작아지기와 납작해지기를 반반(제곱근), 크기는 55% 아래로 안 줄임 (동굴 0.76·0.58, 집 0.57·0.32). 한쪽만 쓰면 너무 작거나 판처럼 납작했다 (`GeckoRig.Squash`, 발밑 기준)
- 몸의 68%가 문 안에 들면 멈추고 엎드린다. 처음에는 서 있는 자세 그대로라 **꼬리가 엉덩이 높이에 곧게 떠서 막대기**처럼 보였다 → `GeckoMotor.SetBurrowed`로 꼬리를 바닥 쪽으로 늘어뜨린다
- 나올 때는 안에서 돌아서서(머리가 밖으로) 문 앞까지, 다 나오면 자르기를 끈다
- 들어갈 쪽: 문이 치우친 쪽 바깥(집 = 왼쪽) · 가운데 문이면 화면 가운데 쪽 · 설 자리가 없으면 반대쪽
- 문 안에 있을 때 게코를 누르면 꼬리는 평소대로, 그 밖은 "누구야?" 하고 나온다 (은신처가 게코 뒤라 게코 터치 영역이 먼저 받는다)
- 문 정보가 없는 은신처는 예전 방식 그대로 (`GoHideBehind`)

**남은 것 (그림):** 집 문이 작아 꽤 움츠린다. 그리고 집 그림은 **문 안쪽 바닥이 투명 + 회색 체크무늬**가 박혀 있다 (그림 생성 때 들어간 자국) — `ART_ORDER_GECKO.md` 8장에 새 그림 규격을 적었다

자가 검사 12개 추가 (목 휨 붙음 · 휨 비율 · 메시가 목 쪽만 되돌림 · 문 자리 2 · 가운데 문 · 움츠림 · 68% · 반대쪽 · 치우친 문 · 작은 문) — **모두 통과 (327개)**

---
## 2026-09-21 — 목 피부 늘어남 · 은신처 작아지기 제거 (둘째 수정)

첫 수정 뒤 플레이 확인: "집에 들어갈 때 갑자기 작아지는 게 이상하다", "고개를 들 때 턱 밑이 갈라져 벌어져 보인다".

### 턱 밑이 벌어짐 — 첫 수정에서 놓친 곳

목 휨은 목 **뒤쪽**(그림 왼쪽)만 몸통에 붙였고, 얼굴은 턱까지 통째로 머리 각도대로 돌았다. 그래서 고개를 들면 턱이 올라가며
**원래 머리에 가려 있던 몸통의 목 앞부분(뾰족한 삼각형)과 그 사이 틈**이 드러났다. 첫 미리보기는 목 뒤쪽만 확대해 봐서 이 자리를 놓쳤다
— 이번에는 몸 전체를 크게 그려 턱 밑까지 확인했다.
- 틈을 머리 맨 아랫줄 색으로 칠하는 방법 → 그 줄이 짙은 윤곽선이라 **검은 띠**가 됐다 (버림)
- **목 피부 띠를 늘린다** (`GeckoNeckBend.THROAT` 18%): 칸마다 꼭짓점 3개(아래 · 목 피부 위 · 위), 아래 가장자리는 몸통에 붙인 채 그 위만 머리를 따라간다.
  그림에 있던 목 피부가 그대로 늘어나 색이 맞는다. 턱 끝(72~90%)은 붙이지 않고(윤곽 번짐 방지), 고개를 숙일 때도 붙이지 않는다(띠 뒤집힘 방지). 12°·14°·18° 모두 틈 없음

### 은신처 — 작아지기 없이

- 크기를 그대로 두면 게코(키 234)가 문(동굴 150 · 집 83)보다 커서 **등과 꼬리가 문 위 바위 속으로 파고들고**, 다 들어간 뒤 꼬리가 바위 한가운데서 나왔다 (미리보기로 확인)
- 즉 **문이 게코보다 작은 한** 자연스럽게 들어갈 방법이 없다 → 문을 게코에 맞춘다
  - **동굴**(코드로 그린 임시 그림): 입구를 곧은 옆면 230 + 둥근 위 45, 폭 280으로 다시 그렸다 (`DrawCave`, 바위도 226×380으로 키움). `doorRect` 0.196~0.804 × 0.10~0.698. 크기 그대로 고개를 6° 숙이고 들어간다
  - **집**(그려진 최종 그림, 문이 게코 키의 35% + 문 안쪽 바닥 투명·체크무늬): `doorRect`를 비워 **뒤에 숨기**. 가운데까지 가면 주둥이가 반대편으로 삐져나왔으므로 주둥이가 집 폭 75% 안쪽에 머무는 곳에서 멈추고 엎드려 꼬리만 삐죽
- 움츠림 코드(`GeckoRig.Squash` · `TopExtent` · `doorFit` · `doorMinShrink`)는 모두 지웠다

자가 검사: 목 피부 3개 추가 · 은신처 문 검사를 크기 그대로 기준으로 바꿈 — **모두 통과 (329개)**

---

## 2026-09-22 — 아이 그림풍 전신 게코 (그림 한 장 스킨)

사용자가 다른 도구(GPT)에서 귀여운 아이 그림풍 크레스티드 전신 그림을 만들었다(`Textures/Gecko/ChildArt/hako_child_base_v1.png`, 1536×1024, 왼쪽을 봄).
"목과 몸이 매끄럽게 하나로 이어진" 그림이라 파츠로 자르지 않고 **한 장 그대로** 쓰기로 했다 — 파츠 조립에서 생기던 턱 밑 틈이 구조적으로 없어진다.
GPT 쪽 작업은 그림을 ChildArt에 복사한 데서 멈춰 있었다 (코드 변경 없음).

### 그림 상태 — 그대로 쓰면 안 됐다
- 배경을 지울 때 남은 **흰 안개 테두리**(알파 1~110이 게코 둘레로 두껍게) · 캔버스 전체의 흰 잡티 → 흙 위에 올리면 뿌옇게 번졌다
- 몸 안쪽도 알파 250~252 → 흙이 살짝 비쳤다
- 윤곽은 알파가 100 → 220으로 한 번에 뛰는 곳 → **120 아래는 지우고 · 120~220은 부드럽게 · 220 위는 불투명**, 가장 큰 덩어리만 남기고, 오른쪽을 보게 뒤집어(스킨 규칙) 여백을 잘라 `hako_child_body.png`(1460×883). 원본은 그대로

### 움직임 — 그림 한 장을 휜다 (`GeckoWholeBend`)
- 몸통 Image에 붙는 메시 효과 — 그림을 28×16 격자로 나눠 꼭짓점마다 **머리 정도**(가로 × 세로 경사, 머리 아래 앞다리가 따라 돌지 않게 세로로도 자른다)와 **꼬리 정도**를 구해 머리 관절(목)·꼬리 뿌리를 중심으로 돌린다. 머리는 받아먹을 때 목을 빼는 위치(`offset`)도 따라간다
- 몸 흔들림·호흡·걷기 오르내림·뛰기·돌기는 몸통 뿌리라 그대로 전체에 들어간다. 꼬리 = 꼬리 각도 − 굽힘 합 × 0.5 (±18°)
- 미리보기: 고개 +16°(올려다보기 최대) · −8°, 꼬리 ±15° 모두 목·턱 밑이 한 장으로 이어져 틈이 없고 앞다리·가슴·뒷발은 제자리
- 목 휨(`GeckoNeckBend`)은 전신 스킨에서 꺼진다

### 보이지 않는 파츠 (`GeckoPartArt.hitSize`)
- 그림이 없고 `hitSize`만 있는 파츠는 그리지 않고 **자리만** 자세를 따라간다 → 부위 터치(눈 · 입 · 다리 4 · 머리 · 꼬리 뿌리/끝) · 말풍선·하트 위치 · 눈 닦기 조준 · 걸음 폭이 모두 예전 코드 그대로 돈다
- 혀·그림자는 Painted 그림을 쓴다 (혀는 화면 크기가 같게 1.335배, 뿌리는 입 앞 끝)
- 크기: 이 그림은 고개 든 자세라 길이 대비 키가 0.60(파츠 그림 0.37) → 어덜트 길이를 520으로 (`referenceWidth` 1797) [TBD]

### 연결
- `Hako > Gecko > ⑤ 아이 그림 전신 스킨 만들기` / `-executeMethod GeckoChildArt.BuildBatch` — 정리 그림(이미 있으면 두기) · `GeckoSkin_Child` · 크레스티드 종 `skin` 연결
- 전용 그림이 없는 종(레오파드·가고일)으로 바꾸면 씬 기본 그림으로 되돌린다 (`GeckoRig.UseDefaultSkin`) — 예전에는 앞 게코의 전용 그림이 남을 수 있었다
- 그림 한 장이라 빠진 것: 표정 · 다리 걸음(→ 아래 플레이 확인에서 뼈대로 넣음) · 허물 조각 표시

자가 검사: `TestWholeBodySkin` 43개 (휘는 영역 · 파츠 숨김 · 목 휨 꺼짐 · 길이 · 네 자세 부위 터치 · 혀 · 기본 그림 복귀) — **모두 통과 (372개)**

### 플레이 확인 — "꼬리와 발과 머리가 전혀 안 움직인다" → 뼈대로 다시

첫 판의 원인 셋 (로그에 오류 없음 — 코드가 그렇게 짜여 있었다):
- **발**: 다리를 휘는 코드가 없었다 (몸 전체 흔들림만)
- **꼬리**: 모터의 꼬리 굽힘 12마디를 **모두 더해** 한 각도로 뿌리에서 돌렸다. 물결이 마디마다 0.42rad씩 어긋나 있어 합이 거의 0 → 거기에 × 0.5 · ±18° → 멈춘 것처럼 보였다
- **머리**: 움직이긴 했지만 평소 3~8°를 머리만 휘니 주둥이가 화면에서 10 남짓 → 고정처럼 보였다

`GeckoWholeBend`를 2D 스키닝으로 다시 짰다 (격자 64×36):
- 꼬리: 가운데 선 사슬 8마디(`wholeTailChain`), 모터 굽힘을 마디마다 받아 끝 쪽 관절부터 돌린다 — 물결이 그대로 보인다
- 머리: 배율 1.5 (±20°)
- 다리: 관절 → 발 뼈의 캡슐 무게 (`wholeLegs`). 미리보기(`GeckoChildArt.PreviewBatch`, 게임과 같은 격자 삼각형)로 다섯 번 고쳤다
  1. 네모 상자 → 발이 다리보다 넓어 상자 가장자리가 발가락을 비스듬히 잘랐다 → 캡슐(발 쪽을 넓게)
  2. 맞닿은 두 발을 반씩 섞음 → 붙어 있는 발가락이 길게 늘어났다
  3. 가까운 발이 차지(섞는 구간을 좁게) → 먼 다리 가장자리가 톱니처럼 찢어졌다 (되돌림)
  4. 먼 다리를 가까운 발만큼 평행 이동 → 먼 다리 윗부분이 옆으로 밀려 찢어졌다 (되돌림)
  5. **먼 다리가 옆 가까운 다리의 관절·각도로 똑같이 돈다** + 먼 다리 어깨 넘어가는 구간을 길게 → 발가락·어깨 모두 깨끗. 먼 앞발은 딛는 박자에 조금 들린다 (걸음처럼 보임)
- 걸음 한 박자에 앞발 86 · 뒷발 75 · 들림 40 (스킨 픽셀), 꼬리 끝 42, 고개 6° → 주둥이 53

자가 검사: 전신 그림 검사 51개 (다리 무게 — 배·관절·꼬리 뿌리는 안 따르고 양 끝 발가락은 온전히 · 걸음 · 꼬리 물결 · 고개) — **모두 통과 (380개)**
...
| 날짜 | 증상 | 원인 | 해결 |
|------|------|------|------|
| 2026-09-22 | 전신 그림 하코의 꼬리·발·머리가 전혀 안 움직임 | 다리를 휘는 코드 없음 · 꼬리 굽힘을 모두 더해 한 각도로(물결이 상쇄돼 0) · 머리만 휘어 움직임이 작음 | 뼈대 스키닝 — 꼬리 사슬 8마디 · 다리 캡슐 4개(먼 다리는 옆 가까운 다리와 함께) · 머리 1.5배 |
| 2026-09-21 | 은신처에 들어갈 때 갑자기 작아짐 | 문이 게코보다 작아 문 높이에 맞춰 움츠리게 했다 | 움츠림 제거 · 동굴 입구를 게코만큼 크게 다시 그림 · 문이 작은 집은 뒤에 숨기 |
| 2026-09-21 | (목 휨 뒤에도) 고개를 들면 턱 밑이 갈라져 벌어짐 | 얼굴이 턱까지 통째로 올라가 가려 있던 몸통 목 앞부분과 틈이 드러남 | 목 아래 피부 띠의 아래 가장자리를 몸통에 붙여 늘어나게 |
| 2026-09-21 | 게코가 은신처에 들어가지 않고 뒤에 숨어 머리가 반대편으로 삐져나옴 | 은신처 그림을 게코 앞에 그려 가림 — 게코가 은신처보다 길다 | 문 가장자리 너머를 잘라 내며 문으로 들어가기 (움츠림 · 엎드려 꼬리 늘어뜨림) |
| 2026-09-21 | 고개를 들면 목이 잘린 것처럼 턱 밑에 틈 | 머리 그림을 통째로 회전 — 아래가 곧게 잘린 그림 | 목 쪽은 몸통에 붙인 채 그림을 휜다 (`GeckoNeckBend`) |
| 2026-09-21 | 홈 아래 양쪽 모서리에 검은 얼룩 | 앞 잎사귀 연출이 버튼에 가려 끝만 보임 · 밝은 흙 위에서 튐 | 앞 잎사귀 삭제 |
| 2026-09-21 | 흙 바닥이 보랏빛 회색, 정글 잎이 어둡고 푸름 | 배경 이미지 알파 0.588 — 뒤의 카메라 하늘색이 비침 | 테마를 입힐 때 불투명으로 |
| 2026-09-21 | 홈 좌우가 잘리고 이름·코인·젬·돌봄 버튼·하단 탭이 사라짐 | Game 창이 Free Aspect + 1.22배 확대 + 옆으로 밀린 채 저장됨 (씬·코드는 멀쩡) | `HakoGameViewFit` — 1080x2400 세로 · 화면에 맞게 |
