...
4. 파츠 경계는 **20~30px 겹치게** 남긴다
   - **머리 (`head.png`)는 목 쪽을 몸통과 넉넉히 겹치게** — 코드가 머리 그림의 목 쪽(왼쪽 12%)은 몸통에 붙여 두고 46%부터 얼굴을 돌린다 (`GeckoNeckBend`, 2026-09-21). 그래서 **눈은 머리 그림 가로 46%보다 오른쪽**에 있어야 표정 그림과 어긋나지 않는다. 턱 밑을 60px쯤 더 이어 그려 두면 고개를 크게 들 때 더 자연스럽다
...
## 7-1. 전신 그림 한 장 (2026-09-22 — 지금 하코)

파츠로 자르지 않고 **그림 한 장**을 쓰는 방법. 목·몸이 이어져 있어 고개를 들어도 틈이 없다. 코드가 머리·꼬리 부분만 휘어 움직인다 (`GeckoWholeBend`).
- 파일: `Textures/Gecko/ChildArt/hako_child_base_v1.png` (원본) → `Hako > Gecko > ⑤`가 `hako_child_body.png`로 정리
- 배경은 **완전 투명**이어야 한다. 배경 지우기 도구가 남긴 흰 안개 테두리·잡티·반투명한 몸(알파 250)은 ⑤가 정리하지만, 처음부터 깨끗한 편이 윤곽이 좋다
- 왼쪽을 봐도 된다 (⑤가 뒤집는다). **네 발끝이 한 줄 바닥에 닿는** 옆모습 · 머리와 앞다리가 세로로 겹치지 않는 자세가 좋다 (머리 영역을 가로·세로로 자른다)
- 다리·꼬리·머리는 코드가 그림을 뼈대로 휘어 움직인다. **네 발이 서로 떨어져 있을수록** 걸음이 깔끔하다 (발가락이 붙어 있으면 먼 다리를 가까운 다리와 같이 움직여야 한다). 꼬리는 몸과 겹치지 않게 뒤로 뻗은 편이 좋다
- 한 장이라 표정(감은 눈·웃는 입)은 없다. 표정이 필요하면 **같은 그림 위에 얹을 눈·입 판**(같은 캔버스, 눈·입 자리만 그린 투명 PNG)을 따로 만들어야 한다
- 그림을 바꾸면 `Editor/Gecko/GeckoChildArt.cs`의 부위 표(눈·입·다리·꼬리·목 자리, 정리된 그림 픽셀)를 다시 재고, 정리 그림 PNG를 지운 뒤 ⑤ → 자가 검사
...
| `decor_cave.png` | 460×460 | `a rock cave with a large dark arched entrance in the middle, moss on top` (아래 10% 투명 여백). **입구는 다 자란 게코가 크기 그대로 들어갈 만큼 — 곧은 옆면 높이 230 이상, 폭 280** (지금 임시 그림 규격. `doorRect`와 맞출 것) |

> 모두 **정면에서 살짝 위에서 본 각도**, 배경 그림과 같은 화풍으로.

---

**은신처 그림의 문 (2026-09-21):** 게코는 문으로 **크기 그대로** 들어간다 — 문 가장자리를 넘는 부분부터 안 보이게 잘라 낸다. 그래서 **문이 다 자란 게코(키 234 · 460 그림 기준 곧은 옆면 230 이상)만큼 커야 한다.** 문보다 게코가 크면 몸이 문 위 바위 속으로 파고드는 것처럼 보인다 (작아지게 해 봤지만 "갑자기 작아져서 이상하다"). 문은 **옆면이 곧은 아치**로(자르는 선이 옆면과 맞게), 문 바닥은 그림이 바닥에 닿는 선(baseline)과 같게, 문 안쪽은 **투명하게 두지 말고 어둡게** 칠한다. 에셋의 `doorRect`(그림 안 비율)를 문 안쪽 사각형에 맞춘다. 지금 **집 그림은 문이 게코 키의 35%이고 문 안쪽 바닥이 투명 + 회색 체크무늬**라 `doorRect`를 비워 두었다 (뒤에 숨는다) — 새로 그리면 `doorRect`만 채우면 들어간다
