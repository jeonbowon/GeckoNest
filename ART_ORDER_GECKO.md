...
4. 파츠 경계는 **20~30px 겹치게** 남긴다
   - **머리 (`head.png`)는 목·턱 밑을 아래·뒤로 넉넉히 (60px 이상) 이어 그린다** (2026-09-21) — 지금 그림은 턱 밑이 곧게 잘려 있어, 고개를 위로 10° 넘게 들면(올려다보기 16° · 하품 14°) 그 직선과 작은 틈이 보인다. 뒷머리 위쪽 등 돌기는 **몸통 쪽 돌기와 겹치지 않게** 머리 쪽을 짧게 — 아래로 숙이면 돌기가 두 겹으로 보인다. 평소 둘러보기는 코드가 ±8° 안으로 막아 두었다 (`GeckoMotor.HEAD_LIMIT`)
