...
**은신처 그림의 문 (2026-09-21):** 게코는 문으로 들어간다 — 문 가장자리를 넘는 부분부터 안 보이게 잘라 낸다. 에셋의 `doorRect`(그림 안 비율)를 문 안쪽 사각형에 맞춘다. **문이 클수록 덜 움츠린다** (지금 집 문은 게코 키의 35%라 들어가면서 꽤 작아지고 납작해진다) — 새로 그릴 때는 문 높이를 은신처 높이의 40% 이상, 문 바닥은 그림이 바닥에 닿는 선(baseline)과 같게. 문 안쪽 바닥은 **투명하게 두지 말고 어둡게 칠한다** (지금 집 그림은 문 안쪽 바닥이 투명 + 회색 체크무늬가 박혀 있다)
